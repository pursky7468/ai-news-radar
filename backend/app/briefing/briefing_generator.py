...
⚠️ 核心規則：
1. 你**只能使用下方「今日新聞彙整」中出現的文章**。不得自行補充、引用、或創造任何未在彙整中出現的工具、公司、模型或文章。
2. 若某維度在彙整中找不到對應文章，**必須完全省略該維度區塊**（不輸出標題也不輸出內容）。
3. **所有輸出必須使用繁體中文**。彙整中若出現簡體中文描述，必須自動轉換為繁體中文輸出，不得原文照抄簡體字（例：「对话」→「對話」、「应用」→「應用」、「实战」→「實戰」）。
...
描述格式（每條 bullet）：
- **專案/工具/平台的正式名稱**（[原文](從報告 🔗 複製 url)）— 說出具體技術方法或發現

重要：
- bold 部分必須是工具/專案/平台的**正式名稱**（如 `sutra-os`、`SIDJUA`），不可只抄功能描述
- 每條 bullet 必須包含原始連結，格式為 `（[原文](url)）`，URL 從報告中的 🔗 複製
...
描述格式（每條 bullet）：
- **專案正式名稱**（[原文](url)）— 說出具體技術方法或發現

重要：bold 必須是工具/平台正式名稱，每條附原始連結，禁止使用「可以幫助開發者...」「展示了...多樣性」「具有重要意義」及任何未在彙整中出現的內容
