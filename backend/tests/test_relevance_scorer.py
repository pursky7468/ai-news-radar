...
def test_high_weight_terms_score_high(scorer: RelevanceScorer):
    post = {"x_post_id": "p1", "content": "AI agent uses tool use and multi-agent orchestration"}
...
def test_generic_terms_score_moderate(scorer: RelevanceScorer):
    post = {"x_post_id": "p2", "content": "New AI model released by OpenAI, uses LLM"}
    result = scorer.score_post(post)
    assert 1.0 <= result["relevance_score"] <= 6.0


def test_no_match_scores_zero(scorer: RelevanceScorer):
    post = {"x_post_id": "p3", "content": "My favourite pasta recipe with tomato sauce"}
    result = scorer.score_post(post)
...
def test_label_agent_assigned(scorer: RelevanceScorer):
    post = {"x_post_id": "p4", "content": "Building an AI agent with tool use and MCP"}
    result = scorer.score_post(post)
...
def test_label_other_fallback(scorer: RelevanceScorer):
    post = {"x_post_id": "p6", "content": "Baking bread at home is very satisfying"}
    result = scorer.score_post(post)
...
def test_config_loads_from_yaml(tmp_path):
    ...
    post = {"x_post_id": "p7", "content": "custom_agent_term in action"}
    result = scorer.score_post(post)
...
def test_config_falls_back_to_defaults(caplog):
    ...
    post = {"x_post_id": "p8", "content": "AI agent demo"}
    result = scorer.score_post(post)
    assert result["relevance_score"] >= 0
...
def test_cache_hit_skips_scoring(news_store, mocker):
    ...
    news_store.upsert_post(
        {
            "x_post_id": "cached_post",
            "author_handle": "user",
            "content": "cached content",
            "url": "https://x.com/i/web/status/cached_post",
    ...
    result = scorer.score_post({"x_post_id": "cached_post", "content": "anything"})
...
def test_cache_miss_triggers_scoring(news_store, mocker):
    scorer = RelevanceScorer(news_store=news_store)
    spy = mocker.spy(scorer, "_compute_score")
    scorer.score_post({"x_post_id": "new_post", "content": "AI agent demo"})
...
def test_is_relevant_true_at_threshold(scorer: RelevanceScorer):
    post = {"x_post_id": "p9", "content": "AI agent uses tool use and multi-agent orchestration MCP RAG"}
    result = scorer.score_post(post)
...
def test_is_relevant_false_below_threshold():
    ...
    post = {"x_post_id": "p10", "content": "AI agent demo"}
    result = scorer.score_post(post)
