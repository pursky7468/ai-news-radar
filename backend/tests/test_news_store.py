...
def make_post(**kwargs):
    defaults = dict(
        ...
        url="https://x.com/i/web/status/post_001",
...
def test_upsert_inserts_new_post(news_store: NewsStore):
    ...
    assert results[0].x_post_id == "post_001"


def test_upsert_deduplicates_by_x_post_id(news_store: NewsStore):
    post = make_post()
    news_store.upsert_post(post)
    # second upsert same x_post_id with updated score
...
def test_query_filters_by_label(news_store: NewsStore):
    news_store.upsert_post(make_post(x_post_id="p1", labels=["ai-agent"], is_relevant=True))
    news_store.upsert_post(make_post(x_post_id="p2", labels=["ai-model"], is_relevant=True))
    results = news_store.query_posts(label="ai-agent")
    assert len(results) == 1
    assert results[0].x_post_id == "p1"


def test_query_filters_by_min_score(news_store: NewsStore):
    news_store.upsert_post(make_post(x_post_id="p1", relevance_score=9.0))
    news_store.upsert_post(make_post(x_post_id="p2", relevance_score=4.0))
...
def test_query_filters_by_date_range(news_store: NewsStore):
    ...
    news_store.upsert_post(make_post(x_post_id="p1", posted_at=now))
    news_store.upsert_post(make_post(x_post_id="p2", posted_at=now - timedelta(days=5)))
    results = news_store.query_posts(
...
def test_query_filters_by_keyword(news_store: NewsStore):
    news_store.upsert_post(make_post(x_post_id="p1", content="multi-agent system demo"))
    news_store.upsert_post(make_post(x_post_id="p2", content="unrelated cooking tips"))
...
def test_mark_digest_sent(news_store: NewsStore):
    news_store.upsert_post(make_post(x_post_id="p1"))
...
def test_get_unsent_relevant_posts(news_store: NewsStore):
    news_store.upsert_post(make_post(x_post_id="p1", is_relevant=True, digest_sent=False))
    news_store.upsert_post(make_post(x_post_id="p2", is_relevant=True, digest_sent=True))
    news_store.upsert_post(make_post(x_post_id="p3", is_relevant=False, digest_sent=False))
